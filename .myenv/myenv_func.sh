...
func_unison_fs_lapmac_all() {
	...
	local disk_candidates="$("ls" /dev/disk*s1 --ignore "disk0s1")"
	...
	[ -e "${mount_path}" ] && [ ! -w "${mount_path}" ] && func_die "ERROR: disk already mount in RO mode, pls eject first!"
...
func_export_script() {
	...
	if [[ "${eresult}" = "success" ]] ; then
		sed -i -e "1i#!/bin/bash\\
		\n# Generated by: $(whoami), at: $(func_dati), git rev: $("cd" "${HOME}" &>/dev/null && git rev-parse HEAD)	\\
