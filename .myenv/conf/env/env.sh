...
addi="$HOME/.myenv/conf/addi"
secu="$HOME/.myenv/secu/"
...
if [[ "$(uname -s)" == CYGWIN* ]] || [[ "$(uname -s)" == MINGW* ]] ; then
	...
	envAliasSrc=(env_alias "${addi}/alias_local" "${secu}/env_alias_secu" env_alias_win)
elif [[ "$(uname -s)" == Darwin* ]] ; then 
	envVarSrc=(env_var env_var_lu env_var_osx)
	envAliasSrc=(env_alias "${addi}/alias_local" "${secu}/env_alias_secu" env_alias_lu env_alias_osx)
else
	envVarSrc=(env_var_lu env_var)
	envAliasSrc=(env_alias "${addi}/alias_local" "${secu}/env_alias_secu" env_alias_lu)
fi
