...
data_validate_kv_file_format() {
	...
	[ "$?" -ne "0" ] && echo "ERROR: format error: no TAB found, or col count inconsistant: $(basename "${1}")" >&2 && exit 1
}

data_validate_kv_file_confliction() {
	awk -F'\t' '
		!seen[$0]++ {					# 过滤掉完全重复的行 (注意这里是"整行比较")
			...
			kv[$1] = $2				# 记录当前 Key 对应的 Value
		}
		END { if (has_error) exit 1 }
	' "${1}"

	[ "$?" -ne "0" ] && echo "ERROR: K:V conflict: $(basename "${1}")" >&2 && exit 1
}

data_validate_kv_file() {
	...
	data_validate_kv_file_format "${1}"
	data_validate_kv_file_confliction "${1}"
}
...
data_kv_lookup() {
	...
	data_validate_kv_file "${kv_table}"
