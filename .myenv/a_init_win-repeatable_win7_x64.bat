...
SET envVarWin=%HOME%\.myenv\env_var_win7_x64
...
FOR /f "tokens=* eol=# delims=;" %%k in (%envVarCommon% %envVarWin% %envVarWinCommon%) do (
	CALL:FUNC_SET_ENV  %%k 
)
ECHO Setting PATH with value=%newPathEnv% (note, if want set on system level, use /M on VISTA or -M on XP)
REM "C:\Program Files\Support Tools\SETX" PATH "%newPathEnv%"
...
:FUNC_SET_ENV
	...
	IF %key%.==PATH. (
		...
		REM "C:\Program Files\Support Tools\SETX" %key% %value%
		SETX %key% "%value%"
